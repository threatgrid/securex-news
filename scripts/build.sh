...
function build-and-publish-package {
  ...
  if [ "${PKG_TYPE}" == "int" ]; then
    ...
    sudo docker run -v /home/travis/build/threatgrid/securex-news:/code/ -e CYBRIC_API_KEY="${CYBRIC_API_KEY}" -e POLICY_ID=JoJaSviQSdSrX2LdiM2KDA -e WORKSPACE=/home/travis/build/threatgrid/securex-news -v /var/run/docker.sock:/var/run/docker.sock --name zeronorth zeronorth/integration:latest python cybric.py
